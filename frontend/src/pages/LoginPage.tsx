...
export default function LoginPage() {
  const { login, handleGoogleAuth, error, isLoading } = useAuthStore()
  ...
  const googleAuth = async (credentials: any) => {
    await handleGoogleAuth(credentials);
    navigate('/dashboard');
  };
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="mt-8 space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <label htmlFor="email" className="block text-sm font-medium text-gray-700">
                  ...
                  <span className="ml-1 inline-block">
                    <div className="relative group">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-400 inline cursor-help" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      <div className="absolute left-0 bottom-full mb-2 w-72 rounded bg-black text-white text-xs p-2 opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none">
                        Si haces parte de la red de relais y actualmente tienes una membresía, usa el mismo correo electrónico de membresía existente y la misma contraseña para acceder
                      </div>
                    </div>
                  </span>
                </label>
              ...
              {isLoading ? (
                <button
                  className="w-full rounded-md bg-gray-500 py-2 px-4 text-white  focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                >
                  Login...
                </button>
              ) : (
